...
What is the difference between "violation" and "limited reach" then? A bot in violation cannot be submitted to the AI because of egregious rule violations. This is not a search limitation. It is a result of a violation of the terms of use (and as such, I fix them right away). A bot that is "limited reach" is not breaking the terms of use, but is deemed so poor because of some questionable content that **PolyBuzz** refuses to deliver it to users (though direct sharing might still work) as doing so would damage the reputation of the search engine and of the platform overall. The owner earns some creation points for the bot, and unlike the violation state, this bot can be submitted to the AI to be played, but since it is deemed severely subpar, **PolyBuzz** won't promote it.
